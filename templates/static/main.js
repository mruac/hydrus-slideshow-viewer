...
$(document).ready(function () {
    ...
    if (localStorage["hideSidebarDelay"] != undefined) {
        ...
        setHideSidebarDelay(localStorage["hideSidebarDelay"]);
    } else {
        $("#sidebarDelay").val("2000");
        setHideSidebarDelay(2000);
    }

    $("#sidebarDelay").keyup(function (event) {
        localStorage.setItem("hideSidebarDelay", $(event.target).val());
        setHideSidebarDelay($(event.target).val());
    });
    ...
    $("#leftSidebarDraggable, #rightSidebarDraggable").on("mousedown", function (e) {
    ...
    $(document).on("mousemove", function (e) {
        if (!(isResizingLSidebar || isResizingRSidebar)) {
            return;
        } else {
    ...
    $("#leftHover, #rightHover").each(function (i, v) {
        $(v).on("mouseover", function (e) {
            if ($(e.target).is("#rightHover")) {
                $(".rightSidebar").show();
            } else {
                $(".leftSidebar").show();
            }
    ...
    $(".leftSidebar, .rightSidebar").each((i, v) => {
...
$("#submit_jump").click(() => {
    ...
    file.jumpToFile(parseInt(y.val()), parseInt(x.val()));
});

$("#jump_files_by").keyup((e) => {
    nav_increment = parseInt($(e.target).val());
});
$("#file_jump_next").click(() => {
    file.navFile(nav_increment);
});
$("#file_jump_previous").click(() => {
    file.navFile(-(nav_increment));
    // file.jumpToFile(parseInt(y.val()), parseInt(x.val()));
});
...
$("#fileCanvas").bind('mousewheel', function (event) {
...
function setHideSidebarDelay(milliseconds) {
    milliseconds = parseInt(milliseconds);
    if (milliseconds > MAX_TIMER_INT) {
        error_textInput($("#sidebarDelay"))
        return;
    }
    console.debug(`sidebar hide delay: ${milliseconds}`);
    if (milliseconds < 0 || milliseconds == NaN) {
        $(".leftSidebar").off("mouseleave");
        $(".rightSidebar").off("mouseleave");
        return;
    } else {
        setTimeout(function () {
            $(".leftSidebar").off("mouseleave");
