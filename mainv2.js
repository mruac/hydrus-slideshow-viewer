...
$(document).ready(function () {
    ...
    $("#committags").on("click", function (e) {
        ...
        if ($("#tagRepositoryList :selected").parent().attr("label") == "tag repositories") {
            data["service_names_to_actions_to_tags"][destTagRepo.text()]["2"] = toAdd; //pend
            //["3"] rescind pending tag
            //Include Pending tags clientFiles[currentPos.y][currentPos.x]["service_names_to_statuses_to_tags"][destTagRepo.text()]["1"] in $("#taglist").val()
            //TODO:$("#taglist").val() order: pending tags first, then display tags
            //["service_names_to_statuses_to_tags"][destTagRepo.text()]["1"]
            //["service_names_to_statuses_to_display_tags"][destTagRepo.text()]["0"]
            if (toDel.length > 0 && $("#commitmsg input").length > 0) { //petition (remove) with message
                $("#commitmsg").hide();
                toDelwithReason = [];
            ...
            } else if (toDel.length > 0 && $("#commitmsg input").length == 0) {
            ...
            // toDelwithReason = [];
            // reason = "to add later"; //ask for message
            // data["service_names_to_actions_to_tags"][destTagRepo.text()]["4"] =
            //     $.each(toDel, function (i, v) {
            //         toDelwithReason.push([v, reason]);
            //     }); //petiton (request to delete)
        }

        $.ajax({
            type: "POST",
            url: clientURL + `/add_tags/add_tags` + clientKey,
            data: data,
            dataType: "json"
        });
    });

    $("#tagRepositoryList").on("change", function (e) {
        try {
            $("#taglist").val(
                clientFiles[currentPos.y][currentPos.x]["service_names_to_statuses_to_display_tags"][
                    $("#tagRepositoryList :selected").text()
                ][0].join('\n'));
        } catch {
            $("#taglist").val("");
        }
        if ($("#tagRepositoryList :selected").text() !== "all known tags") {
            $("#committags").show();
        } else {
            $("#committags").hide();
        }
    ...
    $("#committags").on("click", function () {

    });

    $.ajax({
        url: clientURL + `/add_tags/get_tag_services?` + clientKey,
        dataType: 'json',
        crossDomain: true,
        type: "GET"
    }).done(function (data) {
...
function testClient() {
    $.ajax({
        url: clientURL + `/verify_access_key?` + clientKey,
        dataType: 'json',
        crossDomain: true,
        type: "GET"
    }).done(function () {
        $("#clientStatus").text("Success").css("color", "limegreen");
    }).fail(function () {
...
function cleanTags(urls, response) {
    ...
    $.ajax({
        url: clientURL + `/add_tags/clean_tags?` + clientKey + `&tags=["` + urls[0] + `"]`,
        dataType: "json",
        crossDomain: true,
        type: "GET",
    }).done(function (data) {
...
function getFileMetaData(arrayFileIDs, response) {
    ...
    $.ajax({
        url: clientURL + `/get_files/file_metadata?` + clientKey + `&file_ids=[${arrayFileIDs[0].splice(0, 50)}]`,
        dataType: 'json',
        crossDomain: true,
        type: "GET"
...
function loadFile(fileMetadata) {
    //handle if no tags exist in selected #tagRepositoryList
    try {
        $("#taglist").val(
            fileMetadata["service_names_to_statuses_to_display_tags"][
                $("#tagRepositoryList :selected").text()
            ][0].join('\n'));
    } catch {
        $("#taglist").val("");
...
function createElem(elem, file) {
    $(".dot-elastic").show();
    var url = clientURL + "/get_files/file?" + clientKey + "&file_id=" + file["file_id"];
    //prefill elem
    if (elem == elemP) {
